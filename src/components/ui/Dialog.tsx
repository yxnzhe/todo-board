...
export function Dialog({ open, onClose, title, children, width = 'max-w-lg' }: DialogProps) {
  ...
  return (
    ...
    >
      <div className={`bg-bg-secondary border border-border-light rounded shadow-2xl w-full ${width} mx-4`}>
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-border">
          <h3 className="text-[11px] font-medium tracking-[0.15em] uppercase text-text-secondary">{title}</h3>
          <button onClick={onClose} className="text-text-muted hover:text-text-primary p-1 rounded hover:bg-bg-hover transition-all duration-200 cursor-pointer">
            <X size={13} />
          </button>
        </div>
